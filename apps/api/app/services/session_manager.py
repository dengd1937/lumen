...
class SessionManager:
    ...
    async def start_session(
        ...
        *,
        ...
        Args:
            ...
            inject_directive: Optional test backdoor directive forwarded to the
                LangGraph producer via closure (NNN4: stored per-task, not as
                instance state, so concurrent sessions cannot share/overwrite
                each other's directive).
        ...
        # NNN4: inject_directive is captured in the _run closure, not stored
        # as an instance attribute — each task holds its own directive so
        # concurrent sessions cannot interfere with each other.
    ...
    async def _run(
        ...
        *,
        ...
        T6 NN1+NNN4: inject_directive forwarded to astream_events via closure,
            not stored as instance state -- concurrent sessions each hold their
            own directive and cannot interfere with each other.
        T6 D12.4: explicitly sense ev.type=="error" -> terminal_status="failed".
            LangGraphService converts exceptions to ErrorEvent and returns cleanly
            (no raise), so the except branches below would NOT fire -- this
            ev.type check is the only place to detect the error path.
        """
        ...
        try:
            async for ev in self._langgraph.astream_events(
                ...
                # D12.4: explicitly sense ErrorEvent to set terminal status to
                # "failed".  LangGraphService converts exceptions to ErrorEvent
                # and returns cleanly (no raise), so the except branches below
                # would NOT fire — this is the only place to detect the error path.
                if ev.type == "error":
            ...
            try:
                ...
                # row stays at 'running' permanently. State machine has
                # no back-edge (T9 HIGH), so the same session_id cannot
                # be retried — manual DB cleanup required for that id.
                # TODO: add logger.exception() once logging infra lands.
                pass
