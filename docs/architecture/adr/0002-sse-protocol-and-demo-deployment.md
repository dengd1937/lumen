...
```typescript
| { type: "heartbeat"; server_time: string }   // 仅服务端→客户端，不写 audit_log
```

字段命名约定（与 ADR-0001 D2 隐式修订对齐）：所有 SSE 事件字段使用 snake_case（`event_id` / `session_id` / `node_id` / `conflict_id` / `report_id` / `server_time`），与 Pydantic 默认输出一致，前后端零 transform。ADR-0001 D2 原 schema 中 camelCase 业务字段（`nodeId` / `conflictId` / `reportId`）按本约定全部修正为 snake_case。
...
| 2026-05-07 | 字段命名风格统一：D8.2 `serverTime` → `server_time`；ADR-0001 D2 业务事件字段 `nodeId` / `conflictId` / `reportId` → `node_id` / `conflict_id` / `report_id`（全 snake_case，与 Pydantic 默认输出一致，零 transform）。M1.0 T1 实施时浮现，已在两 reviewer 复审中确认。 | ADR-0001 D2 / ADR-0002 D8.2 |
